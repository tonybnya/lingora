...
(function () {
    ...
    //  Validation
    // -----------------------------------------------------------------------
    function validate(text, languageRaw) {
        const errors = [];
        if (!text || !text.trim()) errors.push('Please enter the text you want to translate.');
        if (!languageRaw || !languageRaw.trim()) errors.push('Please specify a target language.');
        return { valid: errors.length === 0, errors, language: languageRaw.trim() };
    }
    ...
    window.submitTranslation = async function () {
        ...
        const languageRaw = langEl.value;

        const { valid, errors, language } = validate(text, languageRaw);
        if (!valid) {
            notify({
                type: 'error',
                title: 'Invalid input',
        ...
        const t = resultData.translations?.[0] || null;
        let html = `
            <span class="section-label">Results</span>
            <h2 class="section-title">Your <span class="accent">Translation</span></h2>
        ...
        if (t) {
            ...
            html += `<div class="result-empty">No translation was returned.</div>`;
        }
